...
interface CreateVideoModalProps {
  ...
  mode?: 'talking-head' | 'listing' | 'music-video' | 'animated'
  script?: string
...
export default function CreateVideoModal({ isOpen, onClose, videoTitle, startAtComplete = false, startAtLoading = false, videoData, webhookResponse, mode = 'talking-head', script, onConfirmListing, onConfirmMusicVideo, isFreshSubmission = false }: CreateVideoModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (!startAtComplete && !startAtLoading && !videoData) {
        ...
        // Mark as new submission to prevent socket updates from overriding
        isNewSubmissionRef.current = true
        // For music-video and animated mode with fresh submission, set the redirect flag
        // This allows the countdown to run only after explicit form submission
        if ((mode === 'music-video' || mode === 'animated') && isFreshSubmission) {
  ...
  useEffect(() => {
    ...
    // For music-video and animated mode: only respond to socket updates if we have a legitimate submission
    // This prevents socket updates from triggering loading state when modal is just reopened
    if (mode === 'music-video' || mode === 'animated') {
          ...
          setCountdown(prev => {
            if (prev <= 1) {
              ...
              setTimeout(() => {
                ...
                onClose()
                setTimeout(() => {
                  // For listing, music-video, and animated videos, redirect to gallery (create-video page)
                  const targetPath = (mode === 'listing' || mode === 'music-video' || mode === 'animated') ? '/create-video' : '/create-video'
